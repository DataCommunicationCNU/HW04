...
BOOL CFileAppLayer::Receive(unsigned char* ppayload)
{
	...
	if(fapp_hdr->fapp_msg_type == MSG_TYPE_FRAG) 
	{
		...
		else if(fapp_hdr->fapp_type == DATA_TYPE_CONT) 
		{
			...
			memset(GetBuff[fapp_hdr->fapp_seq_num],0,fapp_hdr->fapp_totlen);
		...
		else if(fapp_hdr->fapp_type == DATA_TYPE_END) 
		{
...
UINT CFileAppLayer::FileThread( LPVOID pParam )
{
	...
	if(bFILE==TRUE)
	{
		...
		//////////////////////// fill the blank ///////////////////////////////
		if(dwFileSize < FILE_READ_SIZE)
			tot_seq_num = 1;
		else
			tot_seq_num = (dwFileSize/FILE_READ_SIZE)+1;
		///////////////////////////////////////////////////////////////////////
	}
	else
	{
	...
	while(i<=tot_seq_num+1) //
	{
		...
		if(bSEND == TRUE)
		{
			...
			{
				//////////////////////// fill the blank ///////////////////////////////
				fapp_hdr->m_sHeader.fapp_type = DATA_TYPE_BEGIN;
				fapp_hdr->m_sHeader.fapp_msg_type = MSG_TYPE_FRAG;
				fapp_hdr->m_sHeader.fapp_totlen = dwFileSize;
				fapp_hdr->m_sHeader.fapp_seq_num = tot_seq_num;

				dwRead = fileName.GetLength();
				memcpy(fapp_hdr->m_sHeader.fapp_data,fileName,dwRead);
				fapp_hdr->m_sHeader.fapp_data[dwRead] = '\0';

				GetBuff = (unsigned char**)malloc(sizeof(unsigned char*)*tot_seq_num);
				memset(GetBuff,0,tot_seq_num);
			...
			else if(i!=0 && i<=tot_seq_num)
			{
				...
				fapp_hdr->m_sHeader.fapp_totlen = dwRead;
				fapp_hdr->m_sHeader.fapp_type = DATA_TYPE_CONT;
				fapp_hdr->m_sHeader.fapp_msg_type = MSG_TYPE_FRAG;

				fapp_hdr->m_sHeader.fapp_seq_num = i-1;
				((CIPLayer*)(fapp_hdr->GetUnderLayer()->GetUnderLayer()))->SetFragOff(fapp_hdr->m_sHeader.fapp_seq_num);

				memcpy(fapp_hdr->m_sHeader.fapp_data,pszBuf,dwRead);

				GetBuff[fapp_hdr->m_sHeader.fapp_seq_num] = (unsigned char*)malloc(sizeof(unsigned char)*dwRead);
				memset(GetBuff[fapp_hdr->m_sHeader.fapp_seq_num],0,dwRead);
				memcpy(GetBuff[fapp_hdr->m_sHeader.fapp_seq_num],pszBuf,dwRead);
